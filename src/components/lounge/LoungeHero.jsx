...
const LoungeHero = ({ lounge, isFollowing, onToggleFollow }) => {
  ...
  return (
    <div className="relative w-full h-[500px] md:h-[600px] overflow-hidden bg-card group">
      ...
      <div className="absolute bottom-0 left-0 z-10 max-w-[1600px] mx-auto p-6 md:p-12 w-full">
        <div className="flex flex-col items-start max-w-3xl">
          <h1 className="text-4xl md:text-6xl font-bold leading-tight text-cream drop-shadow-lg mb-4">
            {lounge.name}
          </h1>
          <div className="flex flex-wrap gap-2">
            {lounge.tags.map(tag => (
              <span key={tag} className="bg-brand/20 text-brand-on-dark border border-brand/40 px-4 py-1.5 rounded-full text-sm font-medium backdrop-blur-sm">
                {tag}
              </span>
            ))}
          </div>
        ...
        {currentImage?.caption && (
          <p className="absolute bottom-6 right-6 md:bottom-12 md:right-12 text-xs text-cream bg-espresso/50 backdrop-blur-sm px-3 py-1.5 rounded-full border border-white/10">
            {currentImage.caption}
          </p>
        )}
      </div>
