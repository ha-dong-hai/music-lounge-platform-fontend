...
const VerifyEmailPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-page text-ink" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      <header className="w-full border-b border-line/50 bg-card/90 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 h-20 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3.5">
            <div className="w-10 h-10 rounded-full border border-brand/40 bg-[#1e1813] flex items-center justify-center text-brand-text">
              <Music2 size={20} />
            </div>
            <div className="flex flex-col">
              <span className="text-xl tracking-wider text-ink font-semibold" style={{ fontFamily: "'Playfair Display', serif" }}>Phòng Trà Sài Gòn</span>
              <span className="text-[11px] tracking-[0.2em] text-ink-soft uppercase font-medium">MusicLounge • Khởi Lập 1972</span>
            </div>
          </Link>
        </div>
      </header>
      ...
      <main className="flex-1 flex items-center justify-center px-4 sm:px-6 py-12 lg:py-16 relative">
        <div className="relative w-full max-w-xl mx-auto z-10">
          <div className="bg-page border border-line rounded-2xl shadow-2xl p-8 sm:p-12 lg:p-14 relative">
            <div className="flex flex-col items-center text-center mb-8">
              <div className="relative mb-5">
                <div className="w-16 h-16 rounded-full border border-brand/50 bg-gradient-to-b from-[#251e18] to-page flex items-center justify-center">
                  <Mail size={28} className="text-brand-text" />
                </div>
              </div>
              <span className="text-xs font-semibold tracking-[0.22em] uppercase text-brand-text mb-2 inline-flex items-center gap-1.5">
              ...
              <p className="text-sm leading-relaxed text-ink-soft max-w-md">
                ...
                <span className="font-semibold text-brand-text border-b border-brand/40 pb-0.5">{email}</span>
              </p>
            </div>
            ...
            {isExpired && !apiError && (
              <div role="alert" className="mb-6 rounded-xl p-4 bg-[#2f1f0a] border border-[#b45309] text-[#fef3c7] flex items-start gap-3">
                <Timer size={20} className="flex-shrink-0 mt-0.5 text-brand-text" />
                ...
                <div className="flex justify-between items-center gap-2 sm:gap-3.5 mb-8" role="group" aria-label="6 ô nhập mã xác thực">
                  {digits.map((d, i) => (
                    <input
                      ...
                      className="w-12 h-14 sm:w-14 sm:h-16 text-center text-2xl font-bold bg-card border border-[#4a3d31] rounded-xl text-[#fbf7ee] transition-all focus:border-brand focus:bg-[#231d17] focus:outline-none focus:ring-2 focus:ring-brand/40"
                      style={{ fontFamily: "'Playfair Display', serif" }}
                    />
                  ))}
                </div>

                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4 px-5 rounded-xl border border-line bg-[#120f0c] mb-8">
                  <div className="flex items-center gap-2.5 text-sm text-[#e8ded0]">
                    <Timer size={18} className="text-brand-text" />
                    <span>Thời hạn phong thư:</span>
                    <span className="font-mono font-bold text-brand-text">{expiresAt ? formatCountdown(secondsLeft) : '--:--'}</span>
                  </div>
                  <button
                    type="button"
                    disabled={!isExpired || isResending}
                    onClick={handleResend}
                    className="text-sm font-semibold text-brand-text hover:text-[#fde68a] disabled:text-[#786b5c] disabled:cursor-not-allowed underline-offset-2 enabled:hover:underline transition-colors"
                  >
            ...
            <div className="mt-8 pt-6 border-t border-line/60 flex items-center justify-between text-sm text-ink-soft">
              <Link to="/register" className="inline-flex items-center gap-1.5 text-[#e8ded0] hover:text-brand-text transition-colors">
                <ArrowLeft size={16} />
      ...
      <footer className="w-full border-t border-line/50 bg-card py-8 text-ink-soft text-xs">
