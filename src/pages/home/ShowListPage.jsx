      ...
      <div className="max-w-[1600px] mx-auto px-6 py-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4 min-w-0">
            <Link to="/" className="p-2 hover:bg-brand-hover/20 rounded-full transition-colors flex-shrink-0">
              <ArrowLeft size={24} />
            </Link>
            <div className="min-w-0">
              ...
              <p className="text-sm text-ink-mute mt-0.5">
                ...
                {' · '}
                <Link to="/shows/search" className="text-brand-text hover:underline">tìm theo bộ lọc</Link>
              </p>
            </div>
          </div>

          <div className="flex items-center gap-2 flex-shrink-0">
            <label htmlFor="sortBy" className="text-sm text-ink-mute">Sắp xếp</label>
            <select
              id="sortBy"
              value={sortBy}
              onChange={(e) => doiCachSap(e.target.value)}
              className="px-3 py-2 bg-card border border-line rounded-lg text-sm text-ink focus:outline-none focus:border-brand/50"
            >
        ...
        {isLoading ? (
          <div className="py-24 flex justify-center"><Loader2 size={32} className="animate-spin text-brand-text" /></div>
        ) : shows.length === 0 ? (
          <div className="bg-card border border-line rounded-2xl p-16 text-center">
            ...
            <p className="text-lg font-semibold text-ink mb-1">Chưa có buổi diễn nào đang mở</p>
            <p className="text-sm text-ink-mute">Quay lại sau, hoặc thử tìm theo bộ lọc.</p>
          </div>
        ) : (
