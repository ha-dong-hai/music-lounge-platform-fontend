    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-5 border-b border-gray-800">
          <h2 className="text-lg font-bold text-white">{isEdit ? 'Sửa nghệ sĩ' : 'Thêm nghệ sĩ'}</h2>
        ...
        <form onSubmit={submit} className="p-5 space-y-4 overflow-y-auto">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-gray-500">Tên nghệ sĩ <span className="text-red-400">*</span></label>
          ...
          <div>
            <label className="text-xs text-gray-500">Email liên lạc</label>
          ...
          <div>
            <label className="text-xs text-gray-500">Đường dẫn ảnh đại diện</label>
            <input value={form.avatarUrl} onChange={(e) => set('avatarUrl', e.target.value)} className={inputCls} placeholder="https://..." />
          </div>

          <div>
            <label className="text-xs text-gray-500">Thể loại nhạc</label>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-5 border-b border-gray-800">
          <h2 className="text-lg font-bold text-white truncate">Liên kết của {performer.name}</h2>
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          {links.length === 0 ? (
            ...
            <ul className="space-y-2">
              {links.map((l) => (
                <li key={l.id} className="flex items-center justify-between gap-3 bg-black/40 border border-gray-800 rounded-lg p-3">
                  <div className="min-w-0">
                    <p className="text-white text-sm font-medium">{l.platform}</p>
                    <a href={l.url} target="_blank" rel="noopener noreferrer"
                      className="text-xs text-[#C3B665] hover:underline truncate block">{l.displayName || l.url}</a>
                  </div>
                  <button onClick={() => xoa(l.id)} disabled={isBusy}
                    className="p-2 rounded-lg text-red-400 hover:bg-red-500/10 disabled:opacity-40 flex-shrink-0" title="Xoá">
                    <Trash2 size={14} />
                  </button>
                </li>
              ))}
          ...
          <form onSubmit={them} className="pt-4 border-t border-gray-800 space-y-3">
            <p className="text-xs text-gray-500">Thêm liên kết mới. Muốn sửa một liên kết thì xoá rồi thêm lại.</p>
            <div className="grid grid-cols-2 gap-3">
    ...
    <div className="space-y-6">
      ...
      {performers.length === 0 ? (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-10 text-center">
          <Music2 size={28} className="mx-auto mb-3 text-gray-700" />
          <p className="text-sm text-gray-500">Chưa có nghệ sĩ nào. Thêm nghệ sĩ để dựng line-up cho buổi diễn.</p>
        </div>
      ) : (
            ...
            <div key={p.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              ...
              {(p.genreNames?.length > 0) && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {p.genreNames.map((g) => (
                    <span key={g} className="px-2 py-0.5 rounded-md bg-gray-800 text-gray-400 text-xs">{g}</span>
                  ))}
                </div>
              )}

              {!p.contactEmail && (
                <p className="mt-3 text-xs text-yellow-400/80 flex items-start gap-1.5 leading-relaxed">
                  <AlertTriangle size={12} className="mt-0.5 flex-shrink-0" />
                  Chưa có email liên lạc — nghệ sĩ này không tự xác nhận được tiền donate.
                </p>
              )}
              {p.contactEmail && (
                <p className="mt-3 text-xs text-gray-500 flex items-center gap-1.5 truncate">
              ...
              <div className="mt-4 flex gap-2">
                ...
                <button onClick={() => setLinksOf(p)}
                  ...
                  {p.socialLinks?.length > 0 && <span className="text-gray-500">({p.socialLinks.length})</span>}
                </button>
              </div>
            </div>
          ))}
